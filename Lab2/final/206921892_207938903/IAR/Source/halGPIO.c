...
enum captureNum captureState = first;
...
extern void writeToLCD(const char* str){
    lcd_puts(str);
}
...
extern void enableTimerA1(void){
    TA1CTL |= MC_2 + TACLR;
}

extern void disableTimerA1(void){
    TA1CTL &= ~MC_3;
    TA1CCTL1 = OUTMOD_0;
}
